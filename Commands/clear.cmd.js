const { MessageEmbed, Constants, CommandInteraction } = require("discord.js");
...
module.exports = {
	...
	async execute(interaction) {
		...
		if (
			interaction.member.roles.cache.has(config.helper_role_id) ||
			interaction.member.roles.cache.has(config.admin_role_id) ||
			interaction.member.roles.cache.has(config.owner_role_id)
		) {
